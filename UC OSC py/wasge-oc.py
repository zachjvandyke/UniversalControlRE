    ...
    if (type == "JM"):  # JSON message
        ...
        for id in content:
            if (content["id"] == "UpdateSlave"):  # Mixer answered the first call.
                messagea = messageEncode(fourBytes, "UM",
                                         "\xca\xc5")  # UDP port on the computer to receive UDP data from the mixer
                messageb = messageEncode(fourBytes, "JM",
                                         "{\"id\": \"Subscribe\", \"clientName\": \"Universal Control AI\", \"clientInternalName\": \"ucapp\", \"clientType\": \"PC\", \"clientDescription\": \"Open Control\", \"clientIdentifier\": \"WASGE-OC\", \"clientOptions\": \"perm users levl redu\", \"clientEncoding\": 23117}")
                print(">>> UM + Subscribe")
...
def mixer_connect(ip, port):
    ...
    while MIXER_CONNECTED == True:
        readable, writable, exceptional = select.select((tcp_sock, osc_sock), empty,
                                                        empty)  # Select data from both sockets
        for s in readable:
